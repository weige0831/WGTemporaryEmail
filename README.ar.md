...
### التنظيف التلقائي للسجلات والقرص

- بيانات البريد: تُحذف العناوين المنتهية تلقائيًا ويُقيَّد الإجمالي بـ `max_storage_mb` (1 جيجابايت افتراضيًا) - الأقدم أولًا
- سجلات الحاويات: كل خدمة تدوّر السجلات 10MB × 3 ملفات (30MB كحد أقصى لكل حاوية)
- ذاكرة البناء المؤقتة: `docker system prune` + `docker builder prune` أسبوعيًا (cron)
- ذاكرة apt المؤقتة: `apt-get autoclean` شهريًا (cron)
- journald محدد بـ 200MB؛ ويضبط `setup.sh` كل ما سبق تلقائيًا
...
- يمكن تحديد عدد الصناديق الدائمة عبر `tempmail.max_permanent_addresses` (0 = بلا حد).
